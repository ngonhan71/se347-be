...
const objectController = {
    ...
    getById: async(req, res) => {
        try {
            ...
            const data = await Object.findById(id)
            if (data) {
                res.status(200).json({
                    message: 'success',
                    error: 0,
    ...
    create: async(req, res) => {
        try {
            const { name } = req.body
            const newObject = new Object({name})
    ...
    updateById: async(req, res) => {
        try {
            const { name } = req.body
            const { id } = req.params
            const result = await Object.findByIdAndUpdate(id, {
                name: name,
            }, {new: true})
            if (result) {
    ...
    deleteById: async(req, res) => {
        try {
            const { id } = req.params
            await Flower.updateMany({object: id }, { object: null})
